...
class NotationTrigExponent(CalculatorPlugin):
    """Calculator plugin to allow for exponents on trigonometric functions
    ...
    class CheckPows(ast.NodeTransformer):
        ...
        def visit_Call(self, node: ast.Call) -> ast.Call | Any:
            if isinstance(node.func, ast.BinOp) and isinstance(node.func.op, ast.Pow) and isinstance(node.func.left, ast.Name) and self.calc.chksym(node.func.left.id) and isinstance(self.calc.getsym(node.func.left.id), type) and issubclass(self.calc.getsym(node.func.left.id), sympy.functions.elementary.trigonometric.TrigonometricFunction | sympy.functions.elementary.trigonometric.TrigonometricFunction):
    ...
    def __init__(self):
        super().__init__(self.__class__.__name__, 19)

    def hook(self, calc: Calculator) -> None:
        # Register the toggles for this plugin
        self.register_toggle(calc, "nte", "notation_trig_exponent", True)
        self.checker = NotationTrigExponent.CheckPows(calc)
